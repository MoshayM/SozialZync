...
function ProjectsTab({
  ...
  return (
    <div className="p-5 lg:p-7 max-w-5xl mx-auto space-y-5">
      ...
      {/* Filter row — only shown when there are real projects */}
      {projects.length > 0 && (
        <div className="flex items-center gap-2 mb-2">
          <select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value)}
            className="text-xs font-semibold rounded-xl px-3 py-1.5 appearance-none cursor-pointer outline-none"
...
async function startOAuthFromWizard() {
  try {
    const { data } = await api.channels.getAuthUrl('PUBLISH', '/projects?connected=1') as { data: { url: string } };
