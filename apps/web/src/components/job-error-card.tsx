    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-red-400">{copy.title}</p>
          <p className="text-xs text-red-400/80 mt-0.5">{description}</p>
          <p className="text-xs text-red-400/65 mt-1 italic">{copy.fix}</p>
          ...
          {(showRetry || showReconnect || showRemove || showDetails) && (
            <div className="flex items-center gap-2 mt-3 flex-wrap">
              ...
              {showRetry && (
                <button
                  onClick={onRetry}
                  className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-medium text-red-400 rounded-lg transition-colors hover:bg-white/5"
              ...
              {showRemove && (
                <button
                  onClick={onRemove}
                  className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-medium text-white/65 rounded-lg transition-colors hover:bg-white/5"
          ...
          {showDetails && detailsOpen && (
            <div className="mt-3 space-y-2">
              {error && (
                <div>
                  <p className="text-[10px] font-semibold text-red-400/80 uppercase tracking-wide mb-1">Error message</p>
                  <pre className="text-[11px] rounded-lg px-3 py-2 overflow-x-auto whitespace-pre-wrap font-mono text-red-400/90 max-h-40" style={{ background: 'rgba(239,68,68,.08)', border: '1px solid rgba(239,68,68,.2)' }}>
                    {error}
                  </pre>
                </div>
              )}
              <div>
                <p className="text-[10px] font-semibold text-red-400/80 uppercase tracking-wide mb-1">Technical details</p>
                <pre className="text-[11px] rounded-lg px-3 py-2 overflow-x-auto whitespace-pre-wrap font-mono text-red-400/90 max-h-64" style={{ background: 'rgba(239,68,68,.08)', border: '1px solid rgba(239,68,68,.2)' }}>
