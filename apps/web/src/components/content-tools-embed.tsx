...
import {
  Target, Loader2, ChevronDown, ChevronUp, TrendingUp, Clock,
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-600">
              ...
              {week.cumulativeGrowthEstimate ? ` · ${week.cumulativeGrowthEstimate}` : ''}
            </p>
          </div>
        </div>
...
// ── Strategy Planner (self-contained, local state only) ───────────────────────
    ...
    <div className="space-y-5">
      <div className="grid lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3">
          ...
          {loading && (
            <div className="py-4">
              <LoadingSteps active={loading} steps={[
          ...
          {plan && (
            <div className="space-y-5">
                    ...
                    {plan.milestones.map((m, i) => (
                      <div key={i} className="flex items-start gap-3">
                        <span className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
                          style={{ background: '#ecfdf5', color: '#065f46' }}>W{m.week}</span>
                        <div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {plan.weeklyPlan.map((week, i) => <WeekCard key={i} week={week} />)}
                </div>
              </div>

              <div className="grid sm:grid-cols-2 gap-4">
                <div className="rounded-2xl p-4" style={{ background: '#ecfdf5', border: '1.5px solid #a7f3d0' }}>
                  <h4 className="font-semibold mb-3 text-sm flex items-center gap-2" style={{ color: '#065f46' }}>
                    <CheckCircle2 className="w-4 h-4" /> Success Metrics
                  </h4>
                ...
                {plan.risks && plan.risks.length > 0 && (
                  <div className="rounded-2xl p-4" style={{ background: '#fff7ed', border: '1.5px solid #fed7aa' }}>
                    <h4 className="font-semibold mb-3 text-sm flex items-center gap-2" style={{ color: '#c2410c' }}>
...
type ToolTab = 'tools' | 'strategy';

const SECTION_TABS: Array<{ id: ToolTab; label: string; icon: React.ComponentType<{ className?: string }> }> = [
  { id: 'tools',    label: 'Content Tools',    icon: FileText },
  { id: 'strategy', label: 'Strategy Planner', icon: Target },
];

export function ContentToolsContent() {
  const [section, setSection] = useState<ToolTab>('tools');
  ...
  return (
    <div className="space-y-5">
      ...
      {section === 'tools'    && <ContentPage />}
      {section === 'strategy' && <StrategyPlanner />}
    </div>
  );
}
