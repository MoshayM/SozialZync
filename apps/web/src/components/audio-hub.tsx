...
export function AudioHub() {
  ...
  return (
    <>
      {/* Workflow strip */}
      <div className="sticky top-0 z-10 border-b px-4 sm:px-8 py-2.5"
        style={{ background: 'rgba(250,249,255,0.97)', backdropFilter: 'blur(8px)', borderColor: '#e3ddf8' }}>
        <div className="max-w-5xl mx-auto flex items-center gap-1.5 flex-wrap">
          ...
          {selectedVoiceName ? (
            <button onClick={() => setSection('voices')}
              ...
              style={{ background: '#f5f2fd', color: '#6D4AE0', border: '1.5px solid #d4c9f9' }}>
              <Mic className="w-3 h-3" />{selectedVoiceName}
              <X className="w-3 h-3 ml-0.5 hover:text-purple-900" onClick={e => { e.stopPropagation(); setSelectedVoiceId(null); setSelectedVoiceName(null); }} />
            </button>
          ) : (
            <button onClick={() => setSection('voices')}
              className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold text-gray-400 transition-all hover:bg-gray-50"
              style={{ border: '1.5px dashed #d4c9f9' }}>
          ...
          {selectedTrackTitle ? (
            <button onClick={() => setSection('music')}
              ...
              <X className="w-3 h-3 ml-0.5 hover:text-green-900" onClick={e => { e.stopPropagation(); setSelectedTrackId(null); setSelectedTrackTitle(null); }} />
            </button>
          ) : (
            <button onClick={() => setSection('music')}
              className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold text-gray-400 transition-all hover:bg-gray-50"
          ...
          <button onClick={() => setSection('tools')}
            ...
            style={bothSelected
              ? { background: 'linear-gradient(135deg,#6D4AE0,#7c5ae8)', color: '#fff', boxShadow: '0 2px 8px rgba(109,74,224,.3)' }
              : { background: section === 'tools' ? '#f5f2fd' : '#faf9ff', color: section === 'tools' ? '#6D4AE0' : '#9b8fc4', border: '1.5px solid #e3ddf8' }}>
      ...
      {/* Main content */}
      <div className="px-4 py-6 sm:p-8 max-w-5xl mx-auto space-y-5">
        {/* Header */}
        <div>
          <div className="flex items-center gap-2 mb-1">
            <Headphones className="w-5 h-5" style={{ color: '#6D4AE0' }} />
            <h2 className="text-xl font-extrabold text-gray-900">Audio Studio</h2>
          </div>
          <p className="text-sm text-gray-500">Select a voice, pick background music, then polish with audio tools.</p>
        </div>
