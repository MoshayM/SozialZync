  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl p-4 space-y-3" style={{ border: '1.5px solid #e3ddf8' }}>
        ...
        {/* Row 1: Full-width search input — gets all horizontal space so placeholder is always visible */}
        ...
        {/* Row 2: Source filter (left) + Search action (right) */}
        <div className="flex gap-2">
          <select value={source} onChange={e => setSource(e.target.value)}
            className="flex-1 bg-[#faf9ff] rounded-xl px-3 py-2 text-sm font-medium outline-none min-w-0"
            style={{ border: '1.5px solid #e3ddf8', color: '#374151' }}>
            {SOURCE_OPTIONS.map(o => (
              <option key={o.value} value={o.value}>
                {o.label}{o.key && providers[o.key] && !providers[o.key].available ? ' ✕' : ''}
              </option>
            ))}
          </select>
          <button onClick={() => void search(query)} disabled={loading || !query.trim() || selectedProviderUnavailable}
            className="flex items-center gap-1.5 px-5 py-2 rounded-xl text-sm font-semibold text-white hover:opacity-90 disabled:opacity-50 shrink-0"
        ...
        {/* Trending chips — horizontal scroll row (no wrapping), TikTok/Pinterest pattern */}
        {!searched && !selectedProviderUnavailable && (
          <div className="space-y-1.5">
            <span className="text-xs font-medium text-gray-500">Trending topics</span>
            <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none -mx-1 px-1">
              {TOPIC_SUGGESTIONS.map(t => (
                <button key={t} onClick={() => void trending(t)}
                  className="shrink-0 px-3 py-1.5 rounded-full text-xs font-medium capitalize transition-all hover:bg-purple-50 active:scale-95"
